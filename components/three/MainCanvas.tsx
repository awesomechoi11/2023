        ...
        >
          ...
          >
            ...
            >
              ...
              >
                <LetterStagger>Creating interactive</LetterStagger>
              </div>
              <div>
                <LetterStagger>experiences for the web.</LetterStagger>
              </div>
            </motion.div>
            <motion.div
              style={{
  ...
  return (
    <motion.span className="inline-block">
      {children.split("").map((letter: string, index: number) => {
        if (letter === " ")
          return (
            <span key={letter + index} className="overflow-hidden inline-block">
...
export const useBlobParticleSim = ({
  ...
  function update(delta: number, time: number) {
    ...
    for (let i = 0; i < maxParticlesCount; i++) {
      ...
      // 60 * delta makes it independent of framerate and max min clamps it
      let closeFactor = Math.max(Math.min(0.96 / (60 * delta), 0.96), 0.1);
      particlePositions[i2] *= closeFactor;
      particlePositions[i2 + 1] *= closeFactor;
      ...
      const minRadius2 = 40;
      if (minRadius2 > x * x + y * y && i <= count) resetPosition(i2, time);
    }
  }
